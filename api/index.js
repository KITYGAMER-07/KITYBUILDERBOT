...
// Create a one-time large-file upload URL for media over Vercel's request cap.
...
app.post('/api/storage/upload-url', async (req, res) => {
  ...
  if (!config) {
    return res.status(503).json({
      success: false,
      error: 'Large-file uploads are not configured yet. Add the required environment variables and redeploy.'
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      error: 'Could not prepare the upload. Confirm the media bucket exists and is public.'
    });
  }
});

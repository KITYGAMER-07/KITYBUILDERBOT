...
export class RichMessageService {
  ...
  canUseRichMarkdownForCode(blocks = []) {
    const supportedTypes = new Set([
      'heading', 'paragraph', 'table', 'list', 'details', 'blockquote',
      'pullquote', 'code', 'math', 'divider', 'anchor', 'footer'
    ]);
    return blocks.some(block => block.type === 'code')
      && blocks.every(block => supportedTypes.has(block.type));
  }

  compileRichHtml(blocks = [], { codeRenderer } = {}) {
    let html = '';
    ...
    for (const block of blocks) {
      switch (block.type) {
        ...
        case 'table': {
          const headers = block.headers || [];
          const rows = block.rows || [];
          html += '<table bordered compact>';
          if (headers.length) {
            html += `<tr>${headers.map(cell => `<th>${this.escapeHTML(cell)}</th>`).join('')}</tr>`;
          }
          html += rows.map(row => `<tr>${(row || []).map(cell => `<td>${this.escapeHTML(cell)}</td>`).join('')}</tr>`).join('');
          html += '</table>';
          break;
        }
        case 'list': {
          const tag = block.listType === 'numbered' ? 'ol' : 'ul';
          html += `<${tag}>${(block.items || []).map(item => {
        ...
        case 'details':
          html += `<details><summary>${this.escapeHTML(block.title || 'Click to expand')}</summary>${this.escapeHTML(block.content)}</details>`;
          break;
        case 'blockquote':
          html += `<blockquote${block.expandable ? ' expandable' : ''}>${this.escapeHTML(block.text)}</blockquote>`;
          break;
        case 'pullquote':
          html += `<aside>${this.escapeHTML(block.text)}${block.author ? `<cite>${this.escapeHTML(block.author)}</cite>` : ''}</aside>`;
          break;
        case 'code': {
          if (codeRenderer) {
            html += codeRenderer(block);
            break;
          }
          ...
          const languageClass = language && language !== 'plaintext'
            ...
            : '';
          html += `<pre><code${languageClass}>${this.escapeHTML(block.code)}</code></pre>`;
          break;
        }
        case 'math':
          html += `<tg-math-block>${this.escapeHTML(block.expression)}</tg-math-block>`;
          break;
        case 'divider':
          html += '<hr/>';
          break;
        case 'anchor':
          html += `<a name="${this.escapeAttribute(block.name)}"></a>`;
          break;
        case 'footer':
          html += `<footer>${this.escapeHTML(block.text)}</footer>`;
          break;
        default:
          break;
      }
  ...
  compileMarkdownCodeFence(block = {}) {
    ...
    // A blank line keeps the fenced block separate from a preceding rich HTML
    // element. Rich Markdown accepts the other editor blocks as HTML.
    return `\n\n${fence}${language}\n${code}\n${fence}\n\n`;
  }

  compileRichMarkdown(blocks = []) {
    return this.compileRichHtml(blocks, {
      codeRenderer: (block) => this.compileMarkdownCodeFence(block)
    });
  }

  compileRichMessage(blocks = []) {
    if (this.canUseRichMarkdownForCode(blocks)) {
      return { markdown: this.compileRichMarkdown(blocks) };
    }
    return { blocks: this.compileRichBlocks(blocks) };
  }
  ...
  async sendRichBlocksToChannel({ chatId, blocks = [], keyboard = [], attachments = [] }) {
    const richMessage = this.compileRichMessage(blocks);

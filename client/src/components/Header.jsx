      ...
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="grid w-full grid-cols-[minmax(0,1fr)_auto_auto] items-stretch gap-2 sm:w-auto sm:flex sm:flex-wrap sm:items-center sm:justify-end">
          ...
          {botStatus?.loading ? (
            ...
            <div
              title={botStatus?.message || 'Add TELEGRAM_BOT_TOKEN in Vercel Environment Variables, then redeploy.'}
              className="min-w-0 h-9 flex items-center gap-1.5 px-3 bg-rose-950/30 text-rose-300 border border-rose-500/30 rounded-lg text-xs font-medium"
            >
              <span className="w-2 h-2 rounded-full bg-rose-400" />
              <span className="truncate">Bot not connected</span>
            </div>
          )}

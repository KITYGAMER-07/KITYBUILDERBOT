...
export const MediaEditor = ({ block, onChange }) => {
  ...
  const handleFileSelect = (event) => {
    ...
    if (file.size > MAX_DIRECT_UPLOAD_BYTES) {
      alert('Direct uploads can be up to 4 MB. Use a public URL for larger media.');
      event.target.value = '';
      return;
    }
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
        <input
          ...
          placeholder={`Paste ${block.type} URL (optional)`}
      ...
      <p className="text-[11px] text-slate-500">
        Direct upload: up to 4 MB total per post. For larger files, paste a public URL.
      </p>

...
import { Plus, Trash2, Image, Layers, Upload, X } from 'lucide-react';
...
export const SlideshowEditor = ({ block, onChange }) => {
  ...
  const handleFileSelect = (index, event) => {
    ...
    if (!file.type.startsWith('image/')) {
      alert('Please choose an image file for this slide.');
      event.target.value = '';
      return;
    }
    if (file.size > MAX_DIRECT_UPLOAD_BYTES) {
      alert('Each direct upload can be up to 4 MB. Use an image URL for larger files.');
      event.target.value = '';
      return;
    }
        ...
        {items.map((item, idx) => (
          <div key={idx} className="bg-slate-900 border border-slate-700 rounded-lg p-2.5 space-y-2">
            ...
            <div className="flex min-w-0 items-center gap-2">
              <label className="shrink-0 flex items-center gap-1.5 rounded bg-blue-600 px-2.5 py-1.5 text-xs font-semibold text-white cursor-pointer hover:bg-blue-500 transition">
                <Upload className="w-3.5 h-3.5" /> Upload image
                <input type="file" accept="image/*" onChange={(event) => handleFileSelect(idx, event)} className="hidden" />
              </label>
              {item.localFile && (
                <div className="min-w-0 flex flex-1 items-center justify-between gap-1 rounded border border-emerald-500/30 bg-emerald-950/30 px-2 py-1.5 text-[11px] text-emerald-200">

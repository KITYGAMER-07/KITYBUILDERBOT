    ...
    <div className={`my-2 grid gap-1 rounded-xl overflow-hidden ${
      ...
      {items.map((it, idx) => (
        <div key={idx} className="aspect-square bg-slate-900 relative overflow-hidden">
          {it.url ? (
            <img src={it.url} alt={`Collage ${idx}`} className="w-full h-full object-cover hover:scale-105 transition" />
          ) : (
        ...
        <div key={rIdx} className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${row.length}, minmax(0, 1fr))` }}>
          {row.map((btn, bIdx) => (
            <button
              ...
              className="py-2 px-3 bg-[#2b5278] hover:bg-[#34628f] active:bg-[#204060] text-white text-xs font-semibold rounded-lg shadow transition flex items-center justify-center gap-1.5 text-center truncate"
            >

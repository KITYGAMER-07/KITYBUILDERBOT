...
export const TelegramSimulator = ({
  blocks = [],
  channel = { title: 'Nova Announcements', username: 'nova_channel' },
  onToast
}) => {
    ...
    switch (block.type) {
      ...
      case 'photo': {
        return (
          <div key={block.id || idx} className="my-2 rounded-xl overflow-hidden bg-slate-900 border border-slate-700/60">
            {block.url ? (
              <img src={block.url} alt="Post media" className="w-full max-h-72 object-cover" />
            ) : (
      ...
      case 'video': {
        return (
          <div key={block.id || idx} className="my-2 rounded-xl overflow-hidden bg-slate-950 border border-slate-700/60">
            {block.url ? (
              <video src={block.url} controls className="w-full max-h-72 object-cover" />
            ) : (
      ...
      case 'animation': {
        return (
          <div key={block.id || idx} className="my-2 rounded-xl overflow-hidden bg-slate-900 border border-slate-700/60">
            {block.url ? (
              <img src={block.url} alt="GIF animation" className="w-full max-h-60 object-cover" />
            ) : (
          ...
          <div className="flex items-center justify-between px-1 text-xs text-slate-400">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-[10px] text-slate-400">@{channel.username || 'channel'} • 12.4K subscribers</div>
              </div>
            </div>

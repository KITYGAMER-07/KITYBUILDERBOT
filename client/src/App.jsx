...
const STORAGE_TEMPLATES_KEY = 'novasp_user_saved_templates';

export function App() {
  const [blocks, setBlocks] = useState(sampleTemplates[0].blocks);
  ...
  const loadLocalStorageData = () => {
    try {
      ...
      const storedTemplates = localStorage.getItem(STORAGE_TEMPLATES_KEY);
      if (storedTemplates) {
  ...
  // Template Actions
  const handleLoadTemplate = (tplBlocks) => {
    setBlocks(JSON.parse(JSON.stringify(tplBlocks)));
    triggerHaptic('success');
  ...
  const handleSaveTemplate = (templateData) => {
    ...
    showToast('Template saved to your drafts!', 'success');
    triggerHaptic('success');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col selection:bg-blue-600 selection:text-white">
      ...
      {toast && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-2.5 rounded-xl shadow-2xl text-xs font-semibold backdrop-blur-md border animate-bounce transition bg-slate-900/95 border-slate-700 text-white">
          {toast.type === 'success' ? (
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-4 grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div
          className={`lg:col-span-5 h-[calc(100vh-100px)] sticky top-20 ${

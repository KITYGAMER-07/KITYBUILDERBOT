...
export class RichMessageService {
  constructor(token) {
    this.token = token || store.getBotToken();
  }

  getBaseUrl() {
    const token = this.token || store.getBotToken();
    if (!token) {
    ...
    for (const block of blocks) {
      switch (block.type) {
        ...
        case 'list': {
          richBlocks.push({
            type: 'list',
            list_type: block.listType || 'bullet', // bullet, numbered, checklist
  ...
  /**
   * Helper to format blocks into rich HTML representation for standard telegram clients / fallback
   */
  blocksToHTML(blocks = []) {
    let html = '';
  ...
  async sendToChannel({ chatId, blocks = [], keyboard = [] }) {
    ...
    // Separate bottom keyboard buttons if any
    let inlineKeyboard = [];
    ...
    // Try Telegram Bot API 10.x sendRichMessage first
    try {
      const richPayload = {
        chat_id: chatId,
        blocks: richBlocks
      ...
      console.log(`Attempting sendRichMessage to ${chatId}...`);
      ...
      console.warn(`sendRichMessage failed or not yet supported on this client/token: ${richErr?.response?.data?.description || richErr.message}`);
      console.log('Falling back to multi-format sendMessage with full HTML & Inline Keyboard...');

      // Check if there are primary photos/videos to send first
      ...
      if (slideshowBlock && slideshowBlock.items && slideshowBlock.items.length > 1) {
        ...
        // Send buttons & remainder text if any
      ...
      // Standard Rich HTML sendMessage
